//! IMAP client for Abby's email account. Proton-style defaults: mail.proton.me:993.
...
mod tests {
    ...
    async fn test_imap_connection() {
        if std::env::var("ABBY_IMAP_TEST").is_err() {
            return;
        }
        let host = std::env::var("ABBY_IMAP_HOST").unwrap_or_else(|_| "mail.proton.me".into());
        let port: u16 = std::env::var("ABBY_IMAP_PORT")
            .ok()
            .and_then(|p| p.parse().ok())
            .unwrap_or(993);
        let user = std::env::var("ABBY_IMAP_USER").unwrap_or_default();
        let pass = std::env::var("ABBY_IMAP_PASS").unwrap_or_default();
