...
mod tests {
    ...
    fn test_initial_stage_is_darkness() {
        let tmp = std::env::temp_dir().join("abby_birth_initial");
    ...
    fn test_verify_crypto_without_pubkey_skips_to_awakening() {
        let tmp = std::env::temp_dir().join("abby_birth_skip_verify");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_verify_crypto_with_real_keys() {
        use abby_core::{generate_external_keypair, sign_constitutional_documents, parse_private_key};

        let tmp = std::env::temp_dir().join("abby_birth_verify_real");
        let _ = fs::remove_dir_all(&tmp);
    ...
    fn test_skip_to_life_from_darkness() {
        let tmp = std::env::temp_dir().join("abby_birth_skip_dark");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_skip_to_life_from_awakening() {
        let tmp = std::env::temp_dir().join("abby_birth_skip_awake");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
        fs::create_dir_all(&docs_dir).unwrap();
    ...
    fn test_skip_to_life_noop_when_already_life() {
        let tmp = std::env::temp_dir().join("abby_birth_skip_noop");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_complete_birth_writes_to_database() {
        let tmp = std::env::temp_dir().join("abby_birth_complete");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_double_birth_prevented() {
        let tmp = std::env::temp_dir().join("abby_birth_double");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_complete_birth_twice_prevented() {
        let tmp = std::env::temp_dir().join("abby_birth_complete_twice");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_advance_cognition() {
        let tmp = std::env::temp_dir().join("abby_birth_cognition");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
        fs::create_dir_all(&docs_dir).unwrap();
    ...
    fn test_full_mvp_lifecycle() {
        let tmp = std::env::temp_dir().join("abby_birth_lifecycle");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
        fs::create_dir_all(&docs_dir).unwrap();
