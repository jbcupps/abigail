...
pub enum BirthError {
    ...
    Email(String),
    #[error("Store error: {0}")]
    Store(#[from] abigail_memory::StoreError),
...
mod tests {
    ...
    fn test_generate_identity() {
        let tmp = std::env::temp_dir().join("abigail_birth_gen_identity");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_advance_past_darkness() {
        let tmp = std::env::temp_dir().join("abigail_birth_advance_dark");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
    ...
    fn test_advance_to_connectivity() {
        let tmp = std::env::temp_dir().join("abigail_birth_advance_conn");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
    ...
    fn test_crystallize_soul() {
        let tmp = std::env::temp_dir().join("abigail_birth_crystallize");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
    ...
    fn test_complete_emergence() {
        let tmp = std::env::temp_dir().join("abigail_birth_emergence");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
        let docs_dir = config.docs_dir.clone();
        fs::create_dir_all(&docs_dir).unwrap();
        ...
        for (name, content) in abigail_core::templates::CONSTITUTIONAL_DOCS {
    ...
    fn test_skip_to_life_for_mvp() {
        let tmp = std::env::temp_dir().join("abigail_birth_skip_mvp_v2");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_complete_birth_legacy() {
        let tmp = std::env::temp_dir().join("abigail_birth_complete_legacy");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_double_birth_prevented() {
        let tmp = std::env::temp_dir().join("abigail_birth_double_v2");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_conversation_history() {
        let tmp = std::env::temp_dir().join("abigail_birth_convo");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_crystallize_wrong_stage_fails() {
        let tmp = std::env::temp_dir().join("abigail_birth_crystal_guard");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
    ...
    fn test_complete_emergence_wrong_stage_fails() {
        let tmp = std::env::temp_dir().join("abigail_birth_emerge_guard");
        let _ = fs::remove_dir_all(&tmp);
        let config = test_config(&tmp);
