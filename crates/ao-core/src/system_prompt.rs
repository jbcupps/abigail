...
mod tests {
    ...
    fn test_build_system_prompt_with_docs() {
        let tmp = std::env::temp_dir().join("ao_sysprompt_docs");
    ...
    fn test_build_system_prompt_fallback() {
        let tmp = std::env::temp_dir().join("ao_sysprompt_fallback");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
    ...
    fn test_operational_section_always_present() {
        let tmp = std::env::temp_dir().join("ao_sysprompt_operational");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
