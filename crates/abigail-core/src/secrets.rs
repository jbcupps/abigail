...
mod tests {
    ...
    fn test_secrets_vault_roundtrip() {
        let tmp = std::env::temp_dir().join("abigail_secrets_test_rt");
    ...
    fn test_secrets_vault_empty_load() {
        let tmp = std::env::temp_dir().join("abigail_secrets_test_empty");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
    ...
    fn test_secrets_vault_remove() {
        let tmp = std::env::temp_dir().join("abigail_secrets_test_rm");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
