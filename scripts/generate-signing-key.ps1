...
# The private key should be stored securely OUTSIDE the Abby repository.
# The public key is placed in a protected location for Abby to read.
#
# Prerequisites: Rust toolchain (for the signing tool)
#
# Usage:
#   .\scripts\generate-signing-key.ps1 -OutputDir "C:\SecureKeys\abby"
#
# This creates:
#   - $OutputDir\signing_key.bin (PRIVATE - keep secure, never commit)
#   - $OutputDir\pubkey.bin (PUBLIC - configure in Abby's external_pubkey_path)
...
fn main() {
    ...
    println!("2. Set external_pubkey_path in Abby config to {}/pubkey.bin", output_dir.display());
...
Write-Host "  4. The public key at $OutputDir\pubkey.bin should be read-only for Abby"
Write-Host ""
Write-Host "To configure Abby, set external_pubkey_path in config.json or example.env"
