...
| **LLM support** | OpenAI + local (Ollama/LM Studio) | Anthropic Claude, OpenAI, Ollama, Kimi, MiniMax |
| **Messaging** | None (desktop-only chat) | WhatsApp, Telegram, Discord, Slack, Signal, iMessage, Teams, Matrix, Google Chat, Zalo, WebChat |
| **Execution** | In-process (Tauri) | Docker sandbox + host + device nodes |
| **Skills** | 2 (web-search, proton-mail stub) | 100+ bundled AgentSkills |
| **Voice** | None | Always-on speech (ElevenLabs), Voice Wake, Talk Mode |
| **Browser** | None | Full browser control via accessibility tree |
| **Memory** | SQLite with 3-tier weights | Markdown files + BM25/vector/graph search |
| **Security** | Ed25519 signing, DPAPI, SSRF protection | Docker isolation, allowlists, tool policy, exec approvals |
| **Identity** | Constitutional docs (soul/ethics/instincts), birth sequence | Simple onboarding, per-channel agent config |
| **Multi-agent** | Single agent (Id/Ego routing) | Multi-agent workspaces with per-channel routing |
