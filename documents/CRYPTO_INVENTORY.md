...
- `scripts/prepare_tauri_bundle_config.mjs` injects the updater verification public key only when updater signing is enabled and can add signing-related bundle fields at build time.
- `scripts/generate_tauri_latest_manifest.mjs` emits `latest.json` only for updater-enabled releases and only from artifacts attached to the release.
